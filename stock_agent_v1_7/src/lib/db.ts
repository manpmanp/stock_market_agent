...
export async function insertFundamentals(env: Env, ticker: string, f: FundamentalsRecord): Promise<void> {
  await env.DB.prepare(
    `INSERT INTO fundamentals_snapshot (
       ...
       market_cap, peg_ratio, beta, return_on_assets, ebit, ebitda,
       enterprise_value, total_debt, total_cash, interest_expense
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  )
    .bind(
      ticker,
...
export async function insertValuation(env: Env, ticker: string, v: ValuationRecord): Promise<void> {
  await env.DB.prepare(
    `INSERT INTO valuation_estimates (
       ticker, source, fair_value, target_price, target_low, target_high, rating, num_analysts,
...
export interface UniverseSnapshotRow {
  ...
  // Added for the decision engine's EV/EBIT, Net Debt/EBITDA, interest
  // coverage, Beta, ROA, and analyst-recommendation cross-checks. See
  // migration 0006 and src/decision/factors.ts.
  beta: number | null;
  ...
  // payout_ratio already existed in fundamentals_snapshot (from the v1
  // engine) but wasn't previously pulled into this join -- added here so
  // the decision engine can surface it as a supporting metric too.
  ...
  const { results } = await env.DB.prepare(
    `WITH latest_price AS (
     ...
     SELECT
       ...
       lf.debt_to_equity, lf.dividend_yield, lf.ev_to_ebitda,
       lf.beta, lf.return_on_assets, lf.ebit, lf.ebitda, lf.enterprise_value,
       lf.total_debt, lf.total_cash, lf.interest_expense, lf.payout_ratio,
       lv.fair_value, lv.num_analysts, lv.target_low, lv.target_high, lv.rating,
       lv.rec_strong_buy, lv.rec_buy, lv.rec_hold, lv.rec_sell, lv.rec_strong_sell,
...
export async function pruneOldSnapshots(env: Env, keepDays = 30): Promise<void> {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`DELETE FROM decisions WHERE run_at < ?`).bind(cutoff),
  ]);
}
...
export interface DecisionRow {
  ...
  entryStatus: string;
  factorScoresJson: string;
  factorContributionJson: string;
  warningsJson: string;
  /** EV/EBIT, Net Debt/EBITDA, interest coverage, Beta, ROA, and the
   *  analyst recommendation/target-price spread, as they stood at run
   *  time -- shown on the decision card as supporting context, never fed
   *  into the weighted score. See migration 0006. */
  supportingMetricsJson: string;
}

/** Writes decision rows in batches, one explicit `runAt` shared by the
 *  whole run -- same rationale as insertRankingsBatch above (avoids a
 *  scoring run getting split across a datetime('now') second boundary). */
export async function insertDecisionsBatch(env: Env, rows: DecisionRow[], explicitRunAt: string): Promise<void> {
  const CHUNK = 50;
  for (let i = 0; i < rows.length; i += CHUNK) {
    const chunk = rows.slice(i, i + CHUNK);
    const stmts = chunk.map((row) =>
      env.DB.prepare(
        `INSERT INTO decisions (
           run_at, ticker, horizon, score, decision, confidence, valuation_status, entry_status,
           factor_scores_json, factor_contribution_json, warnings_json, supporting_metrics_json
         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).bind(
        explicitRunAt,
        row.ticker,
        ...
        row.valuationStatus,
        row.entryStatus,
        row.factorScoresJson,
        row.factorContributionJson,
        row.warningsJson,
        row.supportingMetricsJson
      )
    );
    await env.DB.batch(stmts);
  }
...
/** Latest decisions run for one horizon, best score first. */
export async function getLatestDecisions(env: Env, horizon: string, limit = 50): Promise<Record<string, unknown>[]> {
  const { results } = await env.DB.prepare(
    `SELECT * FROM decisions
     WHERE horizon = ? AND run_at = (SELECT MAX(run_at) FROM decisions WHERE horizon = ?)
...
/** Most recent run_at across all horizons (for the /decision-lab header). */
export async function getLatestDecisionRunAt(env: Env): Promise<string | null> {
  const row = await env.DB.prepare(`SELECT MAX(run_at) AS run_at FROM decisions`).first<{ run_at: string | null }>();
...
export interface BacktestRunRow {
  id: number;
  createdAt: string;
  /** scripts/backtest/harness.py's own data/backtest_report.json, parsed --
   *  see migrations/0007_backtest_runs.sql for why this is stored as one
   *  opaque JSON blob rather than a column-per-metric schema. */
  report: unknown;
}

/** Most recent backtest runs pushed by `scripts/backtest.sh` (newest
 *  first), for the /decision-lab "Model Results" tab. Empty until you've
 *  actually run the backtest pipeline on your machine at least once. */
export async function getRecentBacktestRuns(env: Env, limit = 10): Promise<BacktestRunRow[]> {
  const { results } = await env.DB.prepare(
    `SELECT id, created_at, report_json FROM backtest_runs ORDER BY id DESC LIMIT ?`
  )
    .bind(limit)
    .all<{ id: number; created_at: string; report_json: string }>();
