...
// 5y default: the self-relative valuation signal (where today's price sits
// within this stock's own trading range, see scoring/indicators.ts) needs
// enough history to span a market cycle, not just the last two years.
export async function fetchChart(ticker: string, range = "5y", interval = "1d") {
...
export async function fetchQuoteSummary(ticker: string) {
  const modules = [
    ...
    "recommendationTrend",
    "assetProfile",
    // Added for EV/EBIT and interest coverage: Yahoo doesn't expose either
    // as a ready-made field (unlike EV/EBITDA, via defaultKeyStatistics'
    // enterpriseToEbitda), so operatingIncome (used as EBIT) and
    // interestExpense are read from the most recent annual statement here.
...
export interface YahooQuoteSummaryResponse {
  quoteSummary: {
    result: Array<{
      ...
      // Current-month analyst recommendation-trend breakdown. Plain
      // integers, not RawStat -- Yahoo doesn't wrap these counts the way
      // it wraps most numeric fields elsewhere in this response.
      recommendationTrend?: {
        trend?: Array<{
          period?: string; // "0m" = current month, "-1m"/"-2m"/"-3m" = prior months
          strongBuy?: number;
          buy?: number;
          hold?: number;
          sell?: number;
      ...
      // Most recent annual statement first. Only the two fields this
      // project reads (operatingIncome as an EBIT proxy, interestExpense
      // for interest coverage) are typed -- the real response has many more.
      incomeStatementHistory?: {
        incomeStatementHistory?: Array<{
          endDate?: RawStat;
          operatingIncome?: RawStat;
          interestExpense?: RawStat;
        }>;
      };
    }> | null;
    error: { code: string; description: string } | null;
